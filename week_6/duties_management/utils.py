...
def find_soldier_by_id(soldier_id: int, my_data:list) -> dict | None:
    """
...
def is_valid_name(name: str) -> bool:
    ...
    """
    valid_name = True
    if len(name) <= 0 or has_digits_or_symbols_in(name):
        valid_name = False
...
def has_digits_or_symbols_in(soldier_name):
    for char in soldier_name:
        if not char.isalpha():
            return True
    else:
        return False
